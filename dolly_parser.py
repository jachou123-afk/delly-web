...
st.title("🪐 半自動 - 採購報價彙整表 V36")
st.info("✅ 規格：新增【團隊防撞單提醒】、修復貨號誤抓漏洞、支援極簡標籤、重量運費進位、雷射標識別。")
...
# 💡 自動檢查重複邏輯 (檢查貨號或名稱是否已存在)
all_data = get_cached_data()
duplicate_no = None
duplicate_reason = ""

if all_data and (code or name):
    check_code = f"貨號 {code}".strip() if code and len(code) > 2 else None
    check_name = name.strip() if name and len(name) > 2 else None
    
    for i, row in enumerate(all_data):
        if len(row) > 1:
            cell_val = str(row[1]).strip()
            if check_code and check_code in cell_val:
                duplicate_reason = f"貨號：{code}"
...
if duplicate_no:
    st.error(f"🚨 **防撞單提醒**：您輸入的商品（**{duplicate_reason}**）已經有人建檔過了！目前記錄在表格的 **{duplicate_no}**。請確認是否仍要重複存入。")
    ...
    if st.button("💾 儲存並產出進位公式", type="primary"):
        ...
        if sheet:
            try:
                ...
                get_cached_data.clear() # 💡 存檔成功後立刻清除快取，保證下次防撞雷達抓取到最新進度
            except Exception as e:
                st.error(f"儲存失敗：{e}")
