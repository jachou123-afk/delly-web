...
st.title("🪐 半自動 - 採購報價彙整表 V66")
st.info("✅ 規格:【金鑰防護 V3】、【亞克力=彩盒】、【外箱不誤判為產品】、區塊空一行。")
...
def parse_text(text):
    ...
    for line in text_n.split('\n'):
        line_s = line.strip()
        line_clean = re.sub(emoji_pattern, '', line_s).strip()
        
        # 彩盒尺寸:支援「彩盒」「亞克力」「亚克力」開頭
    ...
    # 注意:不再 fallback 用外箱當產品尺寸 (外箱是整箱尺寸,不是產品尺寸)
