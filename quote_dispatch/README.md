...
- [LINE 商品廣告 Computer Use 執行 SOP](LINE_COMPUTER_USE_SOP.md)：執行電腦每次換機／續作先從 GitHub main 重新讀取；發文前第二次檢查不可省略，通過後使用凍結圖文快速工作台。逐款實際查驗後完成並下一則，或保留證據後整批補登；不可將重新執行當成整批重發。
