...
from cost_audit import block, evidence_status
...
def result_row(item, checked):
    ...
    return {"順序": item["order"], "品號": source.get("code") or item["id"],
            ...
            "原文狀態": evidence_status(report if checked.get("report") and not checked.get("error") else None),
            "計算／商品問題": "；".join(dict.fromkeys(issues)) or "—"}
